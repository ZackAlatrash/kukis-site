    ...
    <Section>
      ...
      <Reveal>
        <div className="mx-auto mt-9 flex max-w-[900px] flex-col items-center gap-5 md:flex-row md:gap-9">
          ...
          <div
            id="builtfor-checks-label"
            className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-ink"
          >
            Is this you?
          </div>
          ...
          <div
            role="group"
            aria-labelledby="builtfor-checks-label"
          ...
          {/* fit meter */}
          <div className="mt-6">
            <div className="flex items-center justify-between text-[0.78125rem]">
          ...
          >
            <motion.span
              ...
              className={`font-display text-[1.125rem] font-bold ${
                qualified ? "text-consent-ink" : "text-cocoa"
              }`}
            >
              {verdict}
            </motion.span>
            ...
            <AnimatePresence>
              {qualified && (
                <motion.a
                  ...
                  className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-cherry-deep px-5 py-2.5 text-[0.875rem] font-semibold text-milk transition-[transform,box-shadow] hover:-translate-y-px hover:shadow-[var(--shadow-hover)]"
                >
                  Book a demo
      ...
      <Reveal delay={0.1}>
        <div className="mx-auto mt-7 max-w-[600px]">
          <div className="mb-3 text-center text-[0.78125rem] font-semibold uppercase tracking-[0.12em] text-crumb">
            Not built for
          </div>
          <div className="grid gap-x-8 gap-y-2 sm:grid-cols-2">
            {builtFor.not.items.map((it) => (
              <div key={it} className="flex items-start gap-2.5">
                <X size={15} className="mt-0.5 shrink-0 text-crumb" aria-hidden />
                <span className="text-[0.84375rem] leading-snug text-crumb">{it}</span>
              </div>
            ))}
          </div>
        </div>

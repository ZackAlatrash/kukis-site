...
export function ConsentChip({ className }: { className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-2 rounded-full border border-blueberry/25 bg-blueberry-soft px-3.5 py-2 text-[13px] font-medium",
        className
      )}
    >
      <Cookie size={15} className="text-crumb" aria-hidden />
      Cookies <span className="text-crumb line-through">off</span>
      <span className="text-crumb/50">·</span>
      <Mail size={15} className="text-consent" aria-hidden />
      Marketing <span className="font-bold text-consent">on</span>
    </span>
  );
}
